...
% Regla que verifica si un elemento está en la lista
member(X, [X|_]).
...
% Esto es para buscar indistinguidamente actores o actrices
any(M, A, R) :- actor(M, A, R).
any(M, A, R) :- actress(M, A, R).

/*  1
    2 actores o actrices (A1 y A2) que hayan trabajado juntos en más de una película.
*/
coactors(A1, A2) :-
    % Buscamos películas en las que hayan participado
    any(M1, A1, _), 
    any(M1, A2, _),
    % Deben ser diferentes
    A1 \= A2,
    % Debe existir al menos otra película diferente en la que hayan participado
    any(M2, A1, _), 
    any(M2, A2, _),
    M1 \= M2.


% Consulta: coactors(X,Y).

/*  2
    Todos los actores/actrices que hayan trabajado para un mismo director. 
*/
actors_per_dir(Director, A) :-
    findall(A, (director(M, Director), any(M, A, _)), L),
    write(L).


% Consulta: actors_per_dir(<DIRECTOR>,A).
...
% findall(A,any(barton_fink,A,_),L). Nos dará una lista de actores de la película barton_fink.

% Mide la longitud de una lista
lon([], 0).
lon([_], 1).
lon([_|Xs], N) :-
    lon(Xs,M),
...
% Busca el máximo valor de una lista tupla
max_tupla([X], X).
max_tupla([(A1,N1), (_,N2)|Xs], M) :-
    N1 > N2,
    max_tupla([(A1,N1)|Xs], M).
max_tupla([(_,N1), (A2,N2)|Xs], M) :-
    N1 =< N2,
    max_tupla([(A2,N2)|Xs], M).


% Lista de películas dados un actor y año
movies_per_actor(A, Y, L) :-
    findall(M,(movie(M,Y), any(M, A, _)),L).

% Lista de actores dado un año
actors_per_year(Y,L) :-
    findall(A, (movie(M,Y), any(M,A,_)), L).

% Lista de actores y película en la que participaron dado un año
actor_movies_by_year(Y, L2) :-
    findall((A,L1), (actors_per_year(Y, L), member(A,L), movies_per_actor(A, Y, L1)), L2).

% Cuenta las películas en las que un actor participa
cuentaP([], []).
cuentaP([(A,L)|Xs], [(A,N)|R]) :-
    lon(L,N),
    cuentaP(Xs,R).

% Actores y número de películas dado un año
lista_movies_actor_year(Y, R) :-
    actor_movies_by_year(Y,L),
    cuentaP(L,R).

/*  3
    actor/actriz con mas actuaciones en un año.
*/
actor_actuaciones_year(Y, M) :-
    lista_movies_actor_year(Y, R),
    max_tupla(R,M).


% Guarda en R las coincidencias entre dos listas.
coincidencias(L1, L2, R) :-
    findall(X, (member(X, L1), member(X, L2)), R).

% Devuelve la lista de actores dada una película y/o un director
cuadro_director(D, M, L) :-
    director(M, D), 
    movie(M, _),
    findall(A, (any(M, A, _)), L).

/*  4
    Director con "el mismo cuadro".
*/
mismo_cuadro(D, M1, M2, L) :-
    cuadro_director(D, M1, L1),
    cuadro_director(D, M2, L2),
    M1 \= M2,
    coincidencias(L1,L2,L).

/*  N grados de separación.
 Existe la llamada “Ley de Bacon” o los seis grados
...
% Nos dice si existe una película en donde dos actores hayan participado juntos.
colab(Any1, Any2, M) :-
    Any1 \= Any2,
    any(M, Any1, _),
    any(M, Any2, _).
...
/*  5
 Conectar en N peliculas a dos actores o actrices.
*/
% La distancia (en películas) de dos actores que actuaron en la misma película es de 1.
conectar_actores(Any1, Any2, 1) :-
    colab(Any1, Any2, _).

% Si la distancia es 2, implica que any1 colaboró en una película con otro actor que colaboró con any2
conectar_actores(Any1, Any2, 2) :-
    % Buscamos actor que haya estado con Any1 y con Any2 en dos diferentes películas.
    colab(Any1, Any_aux, M1),
    colab(Any_aux, Any2, M2),
    M1 \= M2.

conectar_actores(Any1, Any2, N) :-
    Any1 \= Any2, 
    conectar_actores(Any1, Any_aux1, A),
    conectar_actores(Any_aux1, Any_aux2, B),
    conectar_actores(Any_aux2, Any2, C),
    N is A + B + C.
