...
% Declaración de los predicados como dinámicos
:- dynamic movie/2.
:- dynamic director/2.
:- dynamic actor/3.
:- dynamic actress/3.
...
% Leer informacion dada una pelicula
informacion_pelicula(NombrePelicula, Informacion) :-
    movie(NombrePelicula, _), % Verificar si la pelicula existe en la base de conocimientos
    findall(Director, director(NombrePelicula, Director), Directores),
    findall(Actor, actor(NombrePelicula, Actor, _), Actores),
    findall(Actriz, actress(NombrePelicula, Actriz, _), Actrices),
    write('\nDirectores'),
    write(Directores),
    write('\n'),
    write('\nActores'),
    ...
    write(Actrices),
    write('\n').
% Caso en el que la pelicula no existe
informacion_pelicula(NombrePelicula, no_disponible) :-
    \+ movie(NombrePelicula, _). % Si la pelicula no existe, devuelve "no_disponible".

% Agregar nueva informacion para una pelicula
nueva_pelicula(M, Y, D) :-
    \+ movie(M, _),                 % Verificar que la pelicula no exista previamente
    assertz(movie(M, Y)),            % Agregar la pelicula
    assertz(director(M, D)).         % Agregar el director

% Modificar la informacion de una pelicula
modificar_pelicula(Nombre, NuevoAnio, NuevoDirector) :-
    % Verificar si la pelicula existe
    movie(Nombre, _),
    % Retractar la informacion existente
    retract(movie(Nombre, _)),
    retract(director(Nombre, _)),
    % Agregar la nueva informacion
    assertz(movie(Nombre, NuevoAnio)),
    assertz(director(Nombre, NuevoDirector)),
    write('Informacion de la pelicula modificada con exito.').
modificar_pelicula(Nombre) :-
    \+ movie(Nombre, _),
    write('La pelicula especificada no existe.').

% Añadir un nuevo actor a la BC
nuevo_actor(Pelicula, Actor, Rol) :-
    assertz(actor(Pelicula, Actor, Rol)).

% Ahora una actriz
nueva_actriz(Pelicula, Actriz, Rol) :-
    assertz(actress(Pelicula, Actriz, Rol)).

% Borrar una pelicula de la BC
borrar_pelicula(M) :-
    retractall(movie(M, _)),         % Eliminar la pelicula
    retractall(director(M, _)),      % Eliminar el director
    retractall(actor(M, _, _)),      % Eliminar todos los actores
    retractall(actress(M, _, _)).    % Eliminar todas las actrices
...
menu :-
    repeat,
    writeln('Menu'),
    writeln('1. Buscar informacion sobre peliculas.'),
    writeln('2. Agregar informacion sbre peliculas.'),
    writeln('3. Modificar informacion sobre peliculas.'),
    writeln('4. Eliminar informacion sobre peliculas.'),
    writeln('5. Asociar un actor a una pelicula (de la opcion 2).'),
    writeln('6. Asociar una actriz a una pelicula (de la opcion 2).'),
    writeln('7. Salir'),
    write('Ingrese el numero de opcion deseada: '),
    read(Opcion),
    ejecutar_opcion(Opcion).

% Encontrar info
ejecutar_opcion(1) :-
    writeln('Has seleccionado la Opcion 1.'),
    writeln('Ingresa el nombre de la pelicula'),
    read(Nombre),
    informacion_pelicula(Nombre, Info),
    write('\n'),
    menu.

% Agregar info
ejecutar_opcion(2) :-
    writeln('Has seleccionado la Opcion 2.'),
    writeln('Ingresa el nombre de la pelicula'),
    read(Nombre),
    writeln('Ingresa el año de la pelicula'),
    read(Anio),
    writeln('Ingresa el director de la pelicula'),
    read(Director),
    nueva_pelicula(Nombre, Anio, Director),
    write('\n'),
    menu.

% Modificar info
ejecutar_opcion(3) :-
    writeln('Has seleccionado la Opcion 3.'),
    writeln('Ingresa el nombre de la pelicula'),
    read(Nombre),
    write('Ingrese el año de la pelicula: '),
    read(NuevoAnio),
    write('Ingrese el director de la pelicula: '),
    read(NuevoDirector),
    modificar_pelicula(Nombre, NuevoAnio, NuevoDirector),
    write('\n'),
    menu.
...
% Borrar info
ejecutar_opcion(4) :-
    writeln('Has seleccionado la Opcion 4.'),
    writeln('Ingresa el nombre de la pelicula'),
    read(Nombre),
...
% 5 & 6: Agregar info
ejecutar_opcion(5) :-
    writeln('Has seleccionado la Opcion 5.'),
    writeln('Ingresa el nombre de la pelicula'),
    read(P),
    writeln('Ingresa el nombre del actor'),
    read(A),
    writeln('Ingresa el nombre del rol'),
    read(R),
    nuevo_actor(P, A, R),
    write('\n'),
    menu.    
...
ejecutar_opcion(6) :-
    writeln('Has seleccionado la Opcion 6.'),
    writeln('Ingresa el nombre de la pelicula'),
    read(P),
    writeln('Ingresa el nombre de la actriz'),
    read(A),
    writeln('Ingresa el nombre del rol'),
    read(R),
    nueva_actriz(P, A, R),
